...
public class Principal extends AppCompatActivity {
    ...
    public void calcular(View v) {
        ...
        String strCantidad = cajaCantidad.getText().toString().trim();
        int genero = comboGenero.getSelectedItemPosition();
        int tipo = comboTipoC.getSelectedItemPosition();
        int marca = comboMarca.getSelectedItemPosition();
        boolean valido = true;
        int pares = 0;

        // Validar
        if (strCantidad.isEmpty()) {
            cajaCantidad.setError(recursos.getString(R.string.error_cant));
            cajaCantidad.requestFocus();
            valido = false;
        } else {
            try {
                pares = Integer.parseInt(strCantidad);

                if(pares == 0) {
                    cajaCantidad.setError(recursos.getString(R.string.error_cant_cero));
                    cajaCantidad.requestFocus();
                    valido = false;
                }
        ...
        if (valido) {
            ...
            double precio = Metodos.calcularPrecio(pares, genero, tipo, marca);
            cajaVrUnitario.setText(String.format("$%.2f", precio));
        }
    }
